...
const intervalExperiment = createIntervalExperiment(({ count }) => {
  message.hidden = false;
  message.textContent = `setInterval test: ${count} callbacks...`;
});
...
document.querySelector("#interval-run").addEventListener("click", (event) => {
  ...
  window.setTimeout(() => {
    const result = intervalExperiment.stop();
    event.currentTarget.disabled = false;
    message.textContent = result
      ? `setInterval: ${result.fps.toFixed(1)} FPS, jitter σ=${result.jitterMs.toFixed(2)} ms, range ${result.minMs.toFixed(2)}–${result.maxMs.toFixed(2)} ms.`
      : "No interval samples collected.";
    dom.intervalFps.textContent = result ? `${result.fps.toFixed(1)} fps` : "—";
    dom.intervalJitter.textContent = result ? `${result.jitterMs.toFixed(2)} ms` : "—";
